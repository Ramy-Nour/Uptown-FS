...
router.post(
  ...
  try {
    ...
    //
    // Prefer an explicit reservation_form_id when provided (from the Contracts
    // console), but fall back to \"by deal_id\" lookup so existing FA flows
    ...
    if (Number.isFinite(reservationFormId) && reservationFormId > 0) {
      try {
        ...
        console.error('Failed to look up reservation form by id for RF:', e)
      }
    }
    ...
    if (!approvedReservation) {
      try {
        ...
        console.error('Failed to look up reservation form by deal_id for RF:', e)
      }
    }

    const calcForLanguage = deal.details?.calculator |_code| new{</}
{}
...
export default router
        ...
        console.error('Failed to look up approved reservation by deal_id:', e)
      }
    }
...
export default router
        ...
        console.error('Failed to look up reservation form by deal for RF:', e)
      }
    }
