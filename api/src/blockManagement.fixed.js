...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const unit = await pool.query('SELECT id, code, available FROM units WHERE id=$1', [unitId])
      if (unit.rows.length === 0) return res.status(404).json({ error: { message: 'Unit not found' } })
      if (unit.rows[0].available === false) return res.status(400).json({ error: { message: 'Unit is not available for blocking' } })

      const existing = await pool.query(
        `SELECT id FROM blocks WHERE unit_id=$1 AND status='approved' AND blocked_until > NOW()`,
        [unitId]
      )
      if (existing.rows.length > 0) return res.status(400).json({ error: { message: 'Unit is already blocked' } })

      // Look up latest approved plan for this unit (by unit_id or by unit_code, whitespace-tolerant)
...
router.patch('/:id/approve', authMiddleware, requireRole(['financial_manager']), validate(blockApproveSchema), async (req, res) => {
  ...
  try {
    const r0 = await pool.query('SELECT * FROM blocks WHERE id=$1', [blockId])
    if (r0.rows.length === 0) return res.status(404).json({ error: { message: 'Block not found' } })
    const row = r0.rows[0]
