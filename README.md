...
- [2025-11-26 21:50] Auto-create approved Payment Plan on block approval (no override)
  - API: Updated the FM block-approval flow in api/src/blockManagement.js so that when a block is approved with financial_decision='ACCEPT' and no override, the system not only auto-approves the latest ACCEPTed deal for that unit (if still draft/pending_approval) but also ensures there is an approved payment_plans row that exactly mirrors the deal snapshot. If no approved payment_plan exists for that deal, a new row is inserted with details copied from deals.details and status='approved'. This guarantees that FA can always see an approved consultant plan for a cleanly-approved block and generate a Reservation Form without any extra manual plan approval step.
