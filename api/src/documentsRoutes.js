      ...
      const buyersHtml = buyers
        .map(
          (b, idx) => `
            ...
            [b.phone_primary, b.phone_secondary].filter(Boolean).join(' / ') ||
            '-'
          }</div>
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // -------------------------
      // FM approval + RF/RESERVED relaxation
      // -------------------------
      ...
      // 3) The unit tied to this deal is currently RESERVED (available = FALSE).
            ...
            `
              ...
              WHERE rf.status = 'approved'
                AND (
                  pp.deal_id = $1
                  OR (
                    rf.details->>'deal_id' ~ '^[0-9]+xpress from 'express'
import puppeteer from 'puppeteer'
import { pool } from './db.js'
import { authMiddleware, requireRole } from './authRoutes.js'
import convertToWords from '../utils/converter.js'
...
export default router
      ...
      // Inputs & helpers
      // -------------------------
      const reservationDateRaw =
        String(req.body?.reservation_form_date || '').trim()
      let reservationDate =
        reservationDateRaw || new Date().toISOString().slice(0, 10)

      // Accept dd/MM/YYYY from UI and normalize to ISO first for display logic
      if (reservationDate.includes('/')) {
        const [dd, mm, yyyy] = reservationDate.split('/')
        if (dd && mm && yyyy) {
          reservationDate = `${yyyy}-${mm.padStart(2, '0')}-${dd.padStart(
            2,
            '0'
          )}`
        }
      ...
      const fmtDate = s => fmtDateDDMMYYYY(s)
      ...
      // Down payment from snapshot (fallback to schedule row)
      let downPayment = Number(calc?.generatedPlan?.downPaymentAmount) || 0
      if (!(downPayment > 0)) {
        try {
          const dpRow = (calc?.generatedPlan?.schedule || []).find(r =>
      ...
      // Pricing strictly from deal snapshot (unitPricingBreakdown)
      const upb = calc?.unitPricingBreakdown || {
        base: 0,
        garden: 0,
        roof: 0,
        ...
        <div class="page">
          ...
          <section class="mb-6 text-lg leading-relaxed ${textAlignLeft}">
            <p>
              ${
                rtl
                  ? `إنه فى يوم <span class="form-input-like px-2">${dayOfWeek || '-'}</span>
                    الموافق <span class="form-input-like px-2">${fmtDate(
                      reservationDate
                    )}</span>
                    تحررت استمارة الحجز الآتي بياناتها:`
                  : `On the day of <span class="form-input-like px-2">${dayOfWeek || '-'}</span>,
                    dated <span class="form-input-like px-2">${fmtDate(
                      reservationDate
                    )}</span>,
                    this reservation form has been issued with the following details:`
              }
          ...
          <section class="mb-8">
            ...
            <div class="space-y-3 text-lg leading-relaxed ${textAlignLeft}">
              <div>
                ...
                <span class="ml-4">
                  ${L('(Unit Area /', '(مساحة الوحدة /')}
                  <span class="form-input-like px-2">
                    ${
                      unit.unit_area != null
                        ? Number(unit.unit_area).toLocaleString(undefined, {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2
                          }) + ' م²'
                        : '-'
            ...
            <div class="mt-4">
              ${
                ...
                <div class="space-y-2 text-base leading-relaxed ${textAlignLeft}">
                  <p>
                    ثمن الوحدة شامل المساحة الإضافية والجراج وغرفة التخزين /
                    ${totalExcl.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} جم
                    (${priceWords} لاغير)
                  </p>
                  <p>
                    وديعة الصيانة /
                  ...
                  <p>
                    إجمالي المطلوب سداده /
                    ${totalIncl.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} جم
                    (${totalWords} لاغير)
                  </p>
                  <p>
                    دفعة حجز مبدئي :
                ...
                <table class="rf-table">
                  ...
                  <tbody>
                    <tr>
                      <td>Base Price (excl. maintenance)</td>
                      <td class="${textAlignRight}">
                        ${totalExcl.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${currency || ''}
                        <div class="text-xs text-gray-600 mt-1">${priceWords}</div>
                      </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                      <td class="font-semibold">Total (incl. maintenance)</td>
                      <td class="${textAlignRight} font-semibold">
                        ${totalIncl.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${currency || ''}
                        <div class="text-xs text-gray-600 mt-1">${totalWords}</div>
                      </td>
                    </tr>
                    <tr>
          ...
          <footer class="mt-10 pt-6 border-t-2 border-gray-800">
            <div class="text-center mb-6 text-base">
              ...
              <span class="form-input-like px-2">${fmtDate(reservationDate) || '-'}</span>
            </div>
            <div class="grid grid-cols-3 gap-8 text-center text-sm md:text-base">
              <div class="flex flex-col items-center">
                <p class="font-bold mb-8">${L('Prepared By', 'إعداد')}</p>
...
export default routerxpress from 'express'
import puppeteer from 'puppeteer'
import { pool } from './db.js'
import { authMiddleware, requireRole } from './authRoutes.js'
import convertToWords from '../utils/converter.js'
