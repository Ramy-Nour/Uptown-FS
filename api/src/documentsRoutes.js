...
export default router
        ...
        ORDER BY rf.approved_at DESC NULLS LAST, rf.id DESC
        LIMIT 1
      `,
        [dealId]
      )
      ...
      console.error('reservation-form lookup error:', e)
    }
    ...
    if (!deal.fm_review_at && !approvedReservation) {
      return bad(res, 403, 'Financial Manager approval required before generating Reservation Form')
    }

    let reservationDate = String(req.body?.reservation_form_date || '').slice(0, 10) || new Date().toISOString().slice(0, 10)
    let preliminaryPayment = null
    const UIcurrency = (req.body?.currency_override || '').trim()
    let language = String(req.body?.language || 'en').toLowerCase().startsWith('ar') ? 'ar' : 'en'

    // If an approved reservation form exists, lock the Preliminary Payment to the value
    // entered by Financial Admin at reservation creation time. The client cannot override it.
    if (approvedReservation) {
      if (approvedReservation.reservation_date) {
        try {
          reservationDate = new Date(approvedReservation.reservation_date).toISOString().slice(0, 10)
        } catch {
          // keep client-provided / default reservationDate on parse failure
        }
      }
      preliminaryPayment = Number(approvedReservation.preliminary_payment) || 0
      if (!req.body?.language && approvedReservation.language) {
        language = String(approvedReservation.language).toLowerCase().startsWith('ar') ? 'ar' : 'en'
      }
    } else {
      // No approved reservation form: keep current behaviour (manual entry from UI).
      preliminaryPayment = Number(req.body?.preliminary_payment_amount) || 0
    }
