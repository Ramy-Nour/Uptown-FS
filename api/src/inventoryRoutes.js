...
          updatedPreview[k] = v
          fields.push(`${k}=$${placeholderCount++}`) // <-- FIX
          params.push(v)
        }
      }
      if (fields.length === 0) { await client.query('ROLLBACK'); client.release(); return bad(res, 400, 'No fields to update') }

      fields.push(`updated_by=$${placeholderCount++}`) // <-- FIX
      params.push(req.user.id)
      
      const idPlaceholder = `$${placeholderCount++}` // <-- FIX
...
  _      WHERE model_id=$1 AND status='approved'
...
    await client.query(
      `INSERT INTO notifications (user_id, type, ref_table, ref