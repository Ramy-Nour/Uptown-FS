...
router.post('/request', authMiddleware, requireRole(['property_consultant','sales_manager']), validate(blockRequestSchema), async (req, res) => {
  ...
  try {
    ...
    // Match either by numeric unit_id stored in the plan snapshot or by unit_code fallback
...
export default router
    ...
    // High-priority control: financial criteria validation at block time
    // Require the approved plan's evaluation.decision === 'ACCEPT'
    try {
      const planDetails = approvedPlan.rows[0].details || {}
      const decision = planDetails?.calculator?.generatedPlan?.evaluation?.decision
      if (decision !== 'ACCEPT') {
        return res.status(400).json({
          error: {
            message: 'Block request denied: payment plan does not meet financial criteria (evaluation decision is not ACCEPT). Request an override.',
            details: { decision: decision || null, plan_id: approvedPlan.rows[0].id }
          }
        })
      }
    } catch (e) {
      // If plan snapshot missing evaluation, treat as invalid for block
      return res.status(400).json({
        error: { message: 'Block request denied: missing financial evaluation in approved plan snapshot. Regenerate plan and re-approve.' }
      })
    })
  } catch (error) {
    console.error('Block request error:', error)
...
export default router
    ...
    const d = Number(durationDays)
    const result = await pool.query(
      `INSERT INTO blocks (unit_id, requested_by, duration_days, reason, status, blocked_until, created_at, updated_at, requested_plan_id, financial_decision, financial_checked_at, override_status)
       VALUES ($1, $2, $3, $4, 'pending', NOW() + ($3::int) * INTERVAL '1 day', NOW(), NOW(), $5, $6, NOW(), $7)
       RETURNING *`,
...
router.patch('/:id/approve', authMiddleware, requireRole(['financial_manager']), validate(blockApproveSchema), async (req, res) => {
  ...
  try {
    ...
    if (action === 'approve') {
      // Guard: financial criteria must pass OR override_status='approved'
