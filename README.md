...
- [2025-11-26 12:00] Calculator Standard Mode + default DP/discount policy
  ...
  - Client: Across all modes, the default sales discount is now 0% instead of 1.5%, and the default Down Payment is expressed as 20% (dpType='percentage', downPaymentValue=20). Target-PV modes no longer auto-convert percentage DP to a fixed amount on mode switch; the DP remains percentage-based by default and the backend solver handles the correct amount.
