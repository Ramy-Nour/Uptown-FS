...
- [2025-11-26 21:30] Reservation Form PDF aligned with multi-owner legal template and server-side data
  - API: Enhanced /api/documents/reservation-form in api/src/documentsRoutes.js so that all client and unit details rendered in the Reservation Form PDF are pulled directly from the deal’s saved calculator snapshot and the units/unit_model_pricing tables (no manual entry by Financial Admin beyond reservation date and preliminary payment). The RF now shows, per buyer (up to 4): name, nationality, ID/passport, ID issue date, address, phones, and email, and uses the stored unit area (units.area) plus detailed pricing breakdown (base, garden, roof, storage, garage, maintenance, totals) to match the paper template structure.
  - API: Extended the unit/pricing lookup for the Reservation Form to select units.area and propagate it into the rendered PDF as “مساحة الوحدة” alongside unit_code and unit_type. This keeps the RF consistent with the approved standard pricing and inventory data without asking the Financial Admin to retype unit or client information.
