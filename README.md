...
- [2026-01-02 01:10] Deal financial summary endpoint and contract DP clause alignment
  ...
  - API: Refined the Arabic contract DP clause in POST /api/generate-document (api/src/app.js) for documentType='contract' so the “remaining down payment” sentence now uses the correct wording “ويتبقى مبلغ … جم يسدد عند توقيع العقد” and continues to bind Arabic placeholders (الدفعة التعاقد بالأرقام, ال دفعة التعاقد كتابتا, بيان الباقي من دفعة التعاقد) to the stored dp totals and remaining amounts derived from reservation_forms.details.dp.
