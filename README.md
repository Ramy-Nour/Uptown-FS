...
- [2026-01-02 23:55] Contracts Queue — fix /api/contracts SQL error and restore navigation header
  - API: Updated api/src/contractsRoutes.js so GET /api/contracts, GET /api/contracts/:id, and GET /api/contracts/candidates join deals via payment_plans (pp.deal_id) instead of a non-existent reservation_forms.deal_id column. This removes the “column rf.deal_id does not exist” (42703) error from the API logs and lets the Contracts Queue load without returning “Internal error” to the client.
