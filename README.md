...
- [2025-11-15 00:00] Fix block request workflow and approved-plan lookup consistency
  - API: Cleaned api/src/blockManagement.js (removed duplicated/merged fragments) and restored the authoritative approved-plan precondition with whitespace-tolerant matching by unit_id or unit_code. This resolves false “An approved payment plan is required…” errors due to mismatched snapshot values and eliminates duplicate route definitions that could cause unpredictable behavior.
  - API: Repaired the regex in GET /api/workflow/payment-plans/approved-for-unit to correctly match numeric unit_id strings (`'^[0-9]+
...
---
`), preventing SQL parse errors and ensuring the client dropdown populates reliably.
  - Impact: Consultants/Sales Managers can request blocks when an approved plan exists; Financial Admin can see approved plans for blocked units in Current Blocks; the Approved Plan dropdown and server checks now agree.
