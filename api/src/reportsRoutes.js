...
router.get(
  '/workflow-logs',
  authMiddleware,
  requireRole(['ceo', 'chairman', 'vice_chairman', 'admin', 'superadmin']),
  async (req, res) => {
    try {
      ...
      if (!type || type === 'offer' || type === 'offers') {
        ...
        if (cid) {
          params.push(cid)
          where.push(`pp.created_by = $${params.length}`)
        }
        if (mid) {
          params.push(mid)
          where.push(
            `pp.created_by IN (SELECT consultant_user_id FROM sales_team_members WHERE manager_user_id=$${params.length} AND active=TRUE)`
          )
        }
        if (start) {
          params.push(start.toISOString())
          where.push(`pp.created_at >= $${params.length}`)
        }
        if (end) {
        ...
        const q = `
          SELECT
            ...
            COALESCE(u.meta->>'name', u.email) AS created_by_name,
            (SELECT stm.manager_user_id
             FROM sales_team_members stm
             WHERE stm.consultant_user_id = pp.created_by
               AND stm.active = TRUE
             LIMIT 1) AS manager_user_id,
            (SELECT um.email
             FROM users um
             WHERE um.id = (
               SELECT stm.manager_user_id
            ...
            (SELECT COALESCE(um.meta->>'name', um.email)
             FROM users um
             WHERE um.id = (
               SELECT stm.manager_user_id
               FROM sales_team_members stm
      ...
      if (!type || type === 'reservation' || type === 'reservations') {
        ...
        if (start) {
          params.push(start.toISOString())
          where.push(`rf.created_at >= $${params.length}`)
        }
        if (end) {
          params.push(end.toISOString())
          where.push(`rf.created_at <= $${params.length}`)
