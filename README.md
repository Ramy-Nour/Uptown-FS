...
- [2026-01-02 23:55] Contracts Queue — fix /api/contracts SQL errors and restore navigation header
  - API: Updated api/src/contractsRoutes.js so GET /api/contracts, GET /api/contracts/:id, and GET /api/contracts/candidates (a) join deals via payment_plans (pp.deal_id) instead of a non-existent reservation_forms.deal_id column, and (b) select unit codes from units.code AS unit_code instead of a non-existent units.unit_code column. This removes both the “column rf.deal_id does not exist” and “column u.unit_code does not exist” (42703) errors from the API logs and lets the Contracts Queue load without returning “Internal error” to the client.
  - Client: Wrapped the Contracts list and Contract Detail pages (client/src/deals/ContractsList.jsx and client/src/deals/ContractDetail.jsx) in the shared BrandHeader layout with proper logout wiring, matching the rest of the Deals app. Contract users now see the standard Uptown Financial System header and shortcuts on /contracts and /contracts/:id instead of a bare, headerless page.
