...
- [2026-01-03 12:15] Contract console follow‑ups: financial summary access, RF PDF for TM, unit metadata, and workflow wiring
  - API: Updated the active financial summary endpoint GET /api/deals/:id/financial-summary in api/src/dealsRoutes2.js (used by app2/index.js) so contract_person, contract_manager, and Top Management roles (ceo/chairman/vice_chairman/top_management) can see any deal’s down-payment summary, not just deals they created. The response now includes both the original Deal Detail field names (maintenance, dp_preliminary_amount, remaining_price_after_dp, etc.) and the Contract Detail aliases (maintenance_deposit, preliminary_amount, remaining_after_dp), so both screens render correctly from a single payload.
  - API: Enriched /api/contracts/:id (api/src/contractsRoutes.js) by joining units and merging structural inventory metadata (unit_area, building_number, block_sector, zone) into contract.unit_info. The Contract Detail “Unit Summary & Delivery” card now shows the same CRM unit data (area, building/block/zone) that appears on the Reservation Form, even when the original calculator snapshot did not capture those fields.
  - API: Extended /api/documents/reservation-form (api/src/documentsRoutes.js) to optionally accept reservation_form_id in addition to deal_id. When called from the Contracts console, the generator first tries the explicit reservation_form_id (status='approved'), then falls back to the existing “by deal_id” lookup. This makes “View Reservation Form PDF” resilient for TM/CM/CA even if deal-to-reservation linkage is unusual, while preserving the Financial Admin workflow.
  - API: Clarified contract approval history by logging distinct change types in contracts_history (create, submit, approve_cm, approve_tm, reject, execute) instead of a single generic “approve”. CM approval now requires status='pending_cm' and sets status='pending_tm'; TM approval requires status='pending_tm' and sets status='approved', matching the intended workflow.
  - Client: Adjusted the Contract Detail unit card (client/src/deals/ContractDetail.jsx) to display Area from unit_info.unit_area (falling back to legacy area/net_area/built_up_area) and a combined “Building / Block / Zone” line using the merged CRM metadata, so CM/TM see the full unit context they expect.
  - Client: Corrected Contract Detail actions so the workflow is: Contract Admin (contract_person) can see the draft and click “Submit to CM (draft → pending CM)” which calls PATCH /api/contracts/:id/submit; Contract Manager sees “Approve (CM → pending TM)” only when status is PENDING_CM and uses PATCH /api/contracts/:id/approve-cm; TM approves with PATCH /api/contracts/:id/approve-tm when status is PENDING_TM. Reject remains available to CM/TM via PATCH /api/contracts/:id/reject, and Contract Admin can still mark an APPROVED contract as EXECUTED (PATCH /api/contracts/:id/execute).
  - Client: Enabled Contract Admin to generate a draft/final Contract PDF directly from Contract Detail for any contract linked to an approved deal, regardless of contract status (DRAFT, PENDING_CM, PENDING_TM, APPROVED, EXECUTED), via POST /api/generate-document with documentType='contract'. “View Reservation Form PDF” on the same screen now passes both deal_id and reservation_form_id to /api/documents/reservation-form, improving reliability for TM/CM/CA while leaving Financial Admin’s Reservation Form generation flow unchanged.

- [2026-01-03 09:30] Contract Manager / Top Management review console for contracts
  - API: Expanded GET /api/deals/:id/financial-summary (api/src/dealsRoutes.js) visibility so contract_person, contract_manager, and Top Management roles (ceo/chairman/vice_chairman/top_management) can load the down-payment financial summary for any deal linked to a contract. Previously this endpoint was restricted to the deal creator plus admin/FM roles, which caused a “Forbidden” error in the Contract Detail “Financial Summary” card when Contract Managers opened contracts created by another user.
  - API: Enriched /api/contracts, /api/contracts/:id, and /api/contracts/candidates (api/src/contractsRoutes.js) to attach buyer_name, client_info, unit_info, and handover_year fields from the original deal and reservation form snapshots, so the Contract Detail console has a complete context (buyer identity, unit code/type/area, and delivery year) even when the contract row itself is minimal.
  - API: Extended /api/documents/reservation-form (api/src/documentsRoutes.js) to accept a deal_id and generate a server-rendered Reservation Form PDF (Arabic/English) that reuses the approved reservation_forms row when present. Role-based access was broadened from financial_admin-only to also allow financial_manager, contract_person, contract_manager, and top-management roles (ceo/chairman/vice_chairman/top_management, plus admin/superadmin) so Contract Admin / CM / TM can open the same Reservation Form document that Financial Admin uses, in a read-only way.
  - Client: Added a dedicated Contract Detail view at /contracts/:id (client/src/deals/ContractDetail.jsx) for contract_person, contract_manager, and TM roles with:
    - Header card showing contract status, linked Deal # and Reservation Form #, unit code, buyer name, created/updated timestamps.
    - Buyer Summary card populated from contract.client_info / reservation form snapshot.
    - Unit Summary & Delivery card showing unit code, type, area, and delivery (handover) year.
    - Financial Summary card that calls GET /api/deals/:dealId/financial-summary and displays total price excluding maintenance, maintenance deposit, total down payment, preliminary/paid/remaining down payment, and remaining price after DP in a read-only layout.
    - Approval History table driven by /api/contracts/:id/history that lists create/approve/reject/execute events with timestamps and actor names.
    - Raw Snapshot section showing the raw JSON from /api/contracts/:id for debugging.
    - Action buttons for submitting a draft contract to CM, CM approval (→ pending TM), TM approval (→ approved), rejection, execution (mark as executed after printing), generating the contract PDF via /api/generate-document, and viewing the Reservation Form PDF via /api/documents/reservation-form. 09:30] Contracts CM/TM review console — buyer/unit summaries, financial visibility, and RF PDF access
  - API: Expanded GET /api/deals/:id/financial-summary (api/src/dealsRoutes.js) visibility so contract_person, contract_manager, and Top Management roles (ceo/chairman/vice_chairman/top_management) can load the down-payment financial summary for any deal linked to a contract. Previously this endpoint was restricted to the deal creator plus admin/FM roles, which caused a “Forbidden” error in the Contract Detail “Financial Summary” card when Contract Managers opened contracts created by another user.
  - API: Enriched contracts queue queries in api/src/contractsRoutes.js so /api/contracts, /api/contracts/:id, and /api/contracts/candidates now join the originating deal snapshot to surface buyer and unit context. Each contract row now includes buyer_name derived from deal.details.calculator.clientInfo.buyer_name (with reservation_form.details.clientInfo as a fallback), a full client_info JSON object, a unit_info snapshot, and handover_year from deal.details.calculator.inputs.handoverYear. This ensures Contract Managers see the buyer name and can read client + unit context even when reservation_forms.details.clientInfo is empty.
  - API: Broadened POST /api/documents/reservation-form (api/src/documentsRoutes.js) role guard to allow financial_manager, contract_person, contract_manager, Top Management, admin, and superadmin to generate Reservation Form PDFs read-only. Contract workflow users can now click “View Reservation Form PDF” from the Contract Detail page without receiving a 403 Forbidden response, while FA retains create/edit control over reservations.
  - Client: Updated Contract Detail UI (client/src/deals/ContractDetail.jsx) to render a “Buyer Summary” card (Name, ID No/Passport, Address, Telephone) from contract.client_info, alongside a “Unit Summary & Delivery” card (Unit Code, Unit Type, Area, Building/Block, Delivery/Handover Year) built from contract.unit_info and handover_year. The existing header “Buyer” label now reflects the imported offer buyer name, giving Contract Manager and Top Management a clear one-screen view of buyer identity, unit data, and delivery year before they approve or reject a contract.
...
- [2026-01-02 06:00] Current Blocks & Deals App small fixes — clean imports and Reservation PDF loading-state bug
  - Client: Removed a stale BrandHeaderLayout import from client/src/deals/App.jsx that referenced a non-existent ../components/BrandHeaderLayout.jsx file, which caused Vite’s import-analysis plugin to fail when building the Deals app. The Deals router now relies solely on BrandHeader for layout, as intended.
  - Client: Fixed a malformed try/catch block in client/src/deals/CurrentBlocks.jsx (printReservationPdf) where a duplicate catch/finally pair had been appended after the function, leading to a “[plugin:vite:react-babel] Unexpected token” parse error at build time. The function now uses a single try/catch/finally that correctly adds the row id to the printing Set on entry and always removes it in finally, preserving the per-row “Generating…” button state without breaking the JS parser.
- [2026-01-02 05:45] Reservation Form Detail page and deep-link from Contract Detail
  - API: Added GET /api/workflow/reservation-forms/:id in api/src/reservationFormsRoutes.js for financial_admin, financial_manager, contract_person, contract_manager, top-management, admin, and superadmin roles. This endpoint returns a single reservation_forms row joined with payment_plans and units, including deal_id, unit_code, unit_type, and buyer_name so it can be inspected without going through the FA/FM queues.
  - Client: Introduced a Reservation Form Detail page at /reservation-forms/:id (client/src/deals/ReservationFormDetail.jsx) that shows a header summary (status, Deal link, Unit, Buyer, Reservation Date), a “View Reservation Form PDF” button wired to generateReservationFormPdf, a Down Payment Breakdown card based on reservation_forms.details.dp (total, preliminary, paid, remaining), and a Raw Snapshot section with the JSON payload for debugging.
  - Client: Updated Contract Detail (client/src/deals/ContractDetail.jsx) to render reservation_form_id as a clickable link to /reservation-forms/:id, so users reviewing a contract can jump directly to the underlying Reservation Form page as well as download its PDF from either screen.
- [2026-01-02 05:30] Contract Detail — direct link back to Reservation Form PDF from contract
  - Client: Updated the Contract Detail page (client/src/deals/ContractDetail.jsx) to display the linked reservation_form_id in the header and add a “View Reservation Form PDF” button whenever a reservation_form_id is present. The button calls generateReservationFormPdf with deal_id and reservation_form_id, downloads the Reservation Form PDF, and shares the same loading state as Contract PDF generation to prevent double-clicks. This gives contract_person/manager one-click access to the underlying RF document without navigating back through the Reservations Queue or Current Blocks.
...
- [2026-01-02 05:00] Contracts Queue Phase 3 — financial snapshot and approval history on Contract Detail
  ...
  - Client: Upgraded the Contract Detail page (client/src/deals/ContractDetail.jsx) with a “Financial Summary” section powered by GET /api/deals/:dealId/financial-summary, showing total price excluding maintenance, maintenance deposit, total down payment, preliminary/paid/remaining down payment, and remaining price after DP in a read-only card that mirrors the Deal Detail financial summary used for contracts.
  - Client: Added an “Approval History” table under Contract Detail that calls GET /api/contracts/:id/history and lists each history entry with timestamp, action, and actor name/email, giving CM/TM and auditors a quick view of how the contract moved through create → approve/reject → execute.
  - Client: Kept the raw JSON payload from /api/contracts/:id in a “Raw Snapshot” section at the bottom of Contract Detail for debugging, while the main content now relies on structured sections (header card, financial summary, and history) for day-to-day use.
- [2026-01-02 04:40] Contracts Queue Phase 2 — status transitions, execution, and Contract PDF generation
  - API: Extended contractsRoutes (api/src/contractsRoutes.js) with GET /api/contracts and GET /api/contracts/:id so the UI can load a queue-style list and individual contracts. The list joins reservation_forms, deals, and units to expose deal_id, unit_code, and buyer_name, and restricts access to contract_person, contract_manager, top-management, admin, and superadmin roles.
  - Client: Enhanced the Contracts list page (client/src/deals/ContractsList.jsx) with a “New Contract from Reservation” action for contract_person roles, which prompts for an approved Reservation Form ID and calls POST /api/contracts. On success it navigates directly to the new contract detail so the CA can continue the workflow.
  - Client: Upgraded the Contract Detail page (client/src/deals/ContractDetail.jsx) from read-only to a basic workflow console: contract_person can submit a draft to CM (draft → pending_tm) and mark approved contracts as executed; contract_manager can approve draft contracts (draft → pending_tm) or reject; and Top Management roles (ceo/chairman/vice_chairman/top_management) can approve pending_tm contracts (pending_tm → approved) or reject. All actions use loading states and refresh the contract snapshot on success.
  - Client: Added a “Generate Contract PDF” button on Contract Detail for approved/executed contracts that calls POST /api/generate-document with documentType='contract' and deal_id, downloads the generated PDF, and shows a success/error toast. This centralizes Contract PDF generation on the Contracts Queue UI while keeping the existing Deal Detail behavior intact.
- [2026-01-02 04:20] Contracts Queue Phase 1 — read-only list and detail views wired into navigation
  - Client: Added a Contracts Queue list page at /contracts (client/src/deals/ContractsList.jsx) that displays a read-only table of contracts returned by GET /api/contracts, including Contract #, Deal #, Unit, Buyer, status chip (Draft/Pending CM/Pending TM/Approved/Executed/Rejected), created timestamp, and a View action that navigates to /contracts/:id.
  - Client: Introduced a Contract Detail page at /contracts/:id (client/src/deals/ContractDetail.jsx) that shows a compact header (status badge, linked Deal #, unit, buyer, created/updated timestamps) plus a JSON snapshot of the raw contract payload. This is intentionally read-only in Phase 1 and will later be replaced by a structured layout and actions (CM/TM approvals, execution, and PDF generation).
  - Client: Wired Contracts navigation and access control by (a) adding “Contracts” to BrandHeader shortcuts for contract_person, contract_manager, and top-management/admin roles (client/src/lib/BrandHeader.jsx), and (b) registering /contracts and /contracts/:id routes in client/src/main.jsx via RoleBasedRoute so only contract_person, contract_manager, ceo, chairman, vice_chairman, top_management, admin, and superadmin can see the queue.
- [2026-01-02 04:00] Deal Detail — disable Client Offer and Contract PDF buttons while documents are generating
  - Client: Updated the Deal Actions bar (client/src/deals/components/DealActionsBar.jsx and client/src/deals/DealDetail.jsx) to track separate loading states for Client Offer PDF and Contract PDF generation. When a Property Consultant clicks “Print Offer (Client Offer PDF)” or a Contract Person clicks “Generate Contract (PDF)”, the respective button now shows a subtle loading state (via LoadingButton) and is disabled until the document-generation call completes, preventing multiple rapid clicks and duplicate exports for the same deal.
  - Client: The underlying generation logic is unchanged: Client Offer PDFs still use the saved calculator snapshot and unit pricing breakdown, and Contract PDFs still call POST /api/generate-document with documentType='contract' via generateDocFromSaved. Only the UX around the buttons has been hardened to match the Reservation Form progress pattern.
- [2026-01-02 03:40] Current Blocks — disable Reservation PDF button while document is generating
  - Client: Updated the Financial Admin “Current Blocks” page (client/src/deals/CurrentBlocks.jsx) to track a per-row “printing” state for the Reservation PDF. When FA clicks “Print Reservation PDF”, the button now changes to “Generating…” and is disabled until the generateReservationFormPdf call completes (success or error), preventing multiple rapid clicks and duplicate document-generation requests for the same reservation.
  - Client: The existing gating rules remain unchanged: the Print button is still disabled unless there is an approved reservation for the selected payment plan, and the API continues to take the reservation date and preliminary payment from the approved reservation row rather than from client input.
- [2026-01-02 03:30] Current Blocks — show full down-payment breakdown fields required by reservation workflow
  - Client: Updated the Financial Admin “Current Blocks” page (client/src/deals/CurrentBlocks.jsx) to render all fields that the reservation workflow now validates: Preliminary Payment Date (business reservation date for the preliminary amount) plus the optional “Paid from Down Payment before reservation” amount and date. These inputs are editable while a reservation request is being prepared, become read-only after FM approval, and feed directly into the POST /api/workflow/reservation-forms payload so the API receives preliminary_payment_date, paid_down_payment_amount, and paid_down_payment_date as expected.
  - Client: Kept the existing behaviour for Reservation Date, Preliminary Payment, language selection, gating on pending/approved reservations, and Block History. The fix removes the confusing “Preliminary Payment Date is required” alert when FA clicks “Create Reservation Form” by ensuring the missing date fields are visible and required in the UI.
...
- [2026-01-02 01:10] Deal financial summary endpoint and contract DP clause alignment
  ...
  - API: Clarified the Arabic contract DP mapping in POST /api/generate-document (api/src/app.js) for documentType='contract' so that «دفعة التعاقد» is treated as the full down payment amount and the remaining portion of the down payment is taken from the same dp breakdown used by Reservation Forms. Contract DOCX templates should bind «الدفعة التعاقد بالأرقام» / «الدفعة التعاقد كتابتا» to the total down payment, and use either «بيان الباقي من دفعة التعاقد» or dp_remaining (and its *_words variant) to display the remaining down payment (باقي المقدم) with whatever exact Arabic sentence is written in the template.
  - Next step (process): Before building the Contracts Queue UI (draft → CM review → TM approval → executed), the team should first test the updated Reservation Form + Contract PDF flows end-to-end on real deals (including split and fully-paid down payments). Once the contract PDFs and DP wording are validated with legal/finance, the next major build is to surface the existing /api/contracts workflow in the UI as a contracts queue for contract_person, contract_manager, and top-management roles.
- [2026-01-02 02:10] Deals API syntax fix and modularization — new clean deals router mounted
  - API: Isolated the Deals router into a new module api/src/dealsRoutes2.js and updated app2.js/app.js to mount it under /api/deals. The new router preserves the existing endpoints used by the UI (listing, /my, /my-proposals, /by-unit/:unitId, GET /api/deals/:id, GET /api/deals/:id/history, GET /api/deals/:id/financial-summary, POST /api/deals, PATCH /api/deals/:id, POST /api/deals/:id/submit, approve/reject, override request, and request-edits/edits-addressed) but is implemented once, cleanly, instead of via the corrupted, duplicated api/src/dealsRoutes.js file.
  - API: As part of this refactor, GET /api/deals/:id/financial-summary now uses a single, well-formed SQL query to locate the latest approved reservation_forms row linked to a deal (either via payment_plans.deal_id or rf.details->>'deal_id' with a safe numeric regex `'^[0-9]+
- [2026-01-02 00:30] Reservation Form DP breakdown and Contract DP data enrichment
  - Client: Extended the Financial Admin “Current Blocks” page (client/src/deals/CurrentBlocks.jsx) so reservation requests now capture a full down-payment breakdown: Preliminary Payment amount and date, an optional “Paid Down Payment before reservation” amount and date, and the business Reservation Form date. After FM approval, all four values render read-only next to the approved plan, while language remains editable for PDFs. The create payload now sends preliminary_payment_date, paid_down_payment_amount, and paid_down_payment_date to the API for each reservation.
  - API: Updated POST /api/workflow/reservation-forms (api/src/reservationFormsRoutes.js) to derive the total down payment from the calculator snapshot (generatedPlan.downPaymentAmount or the “down payment” schedule row), then store a structured dp object in reservation_forms.details with total, preliminary_amount, preliminary_date, paid_amount, paid_date, and remaining (computed as total - preliminary - paid). This makes the Reservation Form the canonical source of down-payment breakdown for both the RF PDF and downstream contract documents, without changing existing reservation status/approval flows.
  - API: Adjusted POST /api/documents/reservation-form (api/src/documentsRoutes.js) so the Arabic financial section uses the stored dp breakdown when available: the Preliminary Payment line shows its own date, the new “Paid from Down Payment” line shows the additional paid part and its date, “Remaining Down Payment” uses dp.remaining, and the global “Remaining Amount” now subtracts the full down payment (dp.total) instead of just the net-of-preliminary value. Older reservations without dp.details still fall back to the previous calculator-based logic, so existing data continues to print correctly.
...
---
`). It returns a normalized summary object (total_excl, maintenance, total_incl, dp_total, preliminary/paid/remaining down payment, and remaining_price_after_dp) or `{ ok: true, summary: null }` if no approved reservation form exists. This replaces the previously broken, partially duplicated implementation that caused “SyntaxError: missing ) after argument list” on API startup.
  - Behaviour: No intentional business-rule changes to the Deals workflow; the goal of this step is to restore API startup, remove the risk of regressing into syntax errors inside a 6k+ line monolithic file, and make future Deals-related changes safer by working in the smaller, modular dealsRoutes2.js file. The legacy api/src/dealsRoutes.js remains in the repo for historical diff only and is no longer imported at runtime.
- [2026-01-02 00:30] Reservation Form DP breakdown and Contract DP data enrichment
  - Client: Extended the Financial Admin “Current Blocks” page (client/src/deals/CurrentBlocks.jsx) so reservation requests now capture a full down-payment breakdown: Preliminary Payment amount and date, an optional “Paid Down Payment before reservation” amount and date, and the business Reservation Form date. After FM approval, all four values render read-only next to the approved plan, while language remains editable for PDFs. The create payload now sends preliminary_payment_date, paid_down_payment_amount, and paid_down_payment_date to the API for each reservation.
  - API: Updated POST /api/workflow/reservation-forms (api/src/reservationFormsRoutes.js) to derive the total down payment from the calculator snapshot (generatedPlan.downPaymentAmount or the “down payment” schedule row), then store a structured dp object in reservation_forms.details with total, preliminary_amount, preliminary_date, paid_amount, paid_date, and remaining (computed as total - preliminary - paid). This makes the Reservation Form the canonical source of down-payment breakdown for both the RF PDF and downstream contract documents, without changing existing reservation status/approval flows.
  - API: Adjusted POST /api/documents/reservation-form (api/src/documentsRoutes.js) so the Arabic financial section uses the stored dp breakdown when available: the Preliminary Payment line shows its own date, the new “Paid from Down Payment” line shows the additional paid part and its date, “Remaining Down Payment” uses dp.remaining, and the global “Remaining Amount” now subtracts the full down payment (dp.total) instead of just the net-of-preliminary value. Older reservations without dp.details still fall back to the previous calculator-based logic, so existing data continues to print correctly.
