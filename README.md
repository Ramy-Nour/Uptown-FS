...
- [2025-10-21 07:45] Inventory approvals — SQL placeholders and query fix:
  - API: Fixed SQL parameter placeholders in Unit Model approval path to use $-placeholders when updating unit_models during Top Management approval (PATCH /api/inventory/unit-models/changes/:id/approve, action 'update').
  - API: Fixed SQL parameter placeholders in Unit Inventory change approval path to use $-placeholders when updating units (PATCH /api/inventory/units/changes/:id/approve, action 'update').
  - API: Removed a stray underscore before WHERE in the approved pricing SELECT used when creating inventory units (POST /api/inventory/units), which caused a syntax error and transaction rollback.
  - Impact: Resolves 500 errors on Approve actions in the Unit Model Approval Queue and Unit Inventory Changes flows.
